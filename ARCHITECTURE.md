...
Orientation for anyone working on Rhema: how the code is laid out, why some things are the way they are, and what to be careful of. Read this before making changes — several decisions here look arbitrary until you know what they cost to get wrong.
...
2. Rebuild the app: `python -m PyInstaller --noconfirm --clean main.spec`.
3. **Launch `dist\Rhema\Rhema.exe` and confirm the Controller and output windows both open.** Not optional, and deliberately before ISCC so a packaging fault costs a rebuild rather than a rebuild plus ~8 minutes of compression. Nothing else in this list can catch one: a source run has every dependency's files sitting on disk, and CI's smoke check is stdlib-only and never builds. v1.1.6 was one step from publishing an app that died inside `ttkb.Style()` on a missing ttkbootstrap asset — before any window existed and before logging was set up, so the only symptom was a process that starts and vanishes, with nothing in the logs. This step is only safe to sit here because `installer.iss` excludes the `settings.json`/`logs`/`*.log` the app writes next to the exe when it runs; without those excludes, verifying a build would package the builder's own machine config into it.
4. Package the installer: `ISCC.exe installer.iss`.
5. Publish a GitHub release tagged `vX.Y.Z` with `dist/Rhema-Setup.exe` attached (`gh release create vX.Y.Z dist/Rhema-Setup.exe --repo TopGradeTech/rhema`) — installed copies won't see the update until this step happens, regardless of what's bumped locally.
6. Confirm the release is not still a draft. `gh release create` shows a large asset's release as a draft with 0 assets while uploading (~21 minutes for 1.6 GB), then publishes it at the end. A draft is invisible to the update check, so verify `isDraft=false` with the asset attached rather than assuming the upload finished. Worth checking the tag landed on `main` too, and that the tree it points at is the one the installer was built from.
