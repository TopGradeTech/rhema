...
class SettingsUIMixin(SettingsLogicMixin):
    ...
    def _show_hardware_autodetect_result(self, text):
        # Overridable hook (see WebSettingsUIMixin) so
        # _run_hardware_autodetect_from_menu itself stays free of a direct
        # tkinter.messagebox call - the real Tk behavior, preserved exactly.
        parent = self.settings_window if self.settings_window is not None else self.root
        messagebox.showinfo("Hardware Autodetect", text, parent=parent)
