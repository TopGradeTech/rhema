...
class MonitorMixin:
    ...
    # ------------------------------------------------------------------ #
    # Video Overlay mode: a chroma-key caption bar docked to the top/bottom
    # of the selected monitor, sized for `overlay_max_lines`, for OBS/RTSP
    # capture. This is a second geometry/style profile for the same
    # `self.root` window used by fullscreen mode, not a separate window.
    # ------------------------------------------------------------------ #

    def _enter_configured_display_mode(self):
    ...
    def toggle_output_mode(self):
        ...
        self.display_mode = "overlay" if previous_mode == "fullscreen" else "fullscreen"
        self.apply_colors()
        self.update_display()
        self._apply_display_mode(previous_mode=previous_mode)
        self.save_settings()

    def _apply_display_mode(self, previous_mode=None):
        if previous_mode == "overlay" and self.display_mode != "overlay":
            self.exit_overlay_mode()
        elif previous_mode == "fullscreen" and self.display_mode != "fullscreen":
    ...
    def enter_overlay_mode(self):
        ...
        if self.prev_geometry is None:
            self.prev_geometry = self.root.geometry()
        self._prepare_overlay_window_state()
        self._apply_overlay_geometry()
        self.caption_overlay_active = True
        self._apply_canvas_padding()

    def _prepare_overlay_window_state(self):
        if self.prev_overrideredirect is None:
            try:
                self.prev_overrideredirect = bool(self.root.overrideredirect())
            except Exception:
    ...
    def _compute_overlay_bar_geometry(self, monitor_index):
        self.monitors = self.get_monitors()
        if not self.monitors:
            return None
        idx = max(0, min(monitor_index, len(self.monitors) - 1))
        ...
        mon_height = max(1, monitor["bottom"] - monitor["top"])
        lines = max(1, int(self.overlay_max_lines))
        line_height = max(1, round(mon_height * self.OVERLAY_LINE_HEIGHT_RATIO))
        bar_height = max(self.OVERLAY_MIN_HEIGHT_PX, line_height * lines)
        bar_height = min(bar_height, mon_height, round(mon_height * self.OVERLAY_MAX_HEIGHT_RATIO))
        x = monitor["left"]
        if self.overlay_position == "top":
            y = monitor["top"]
        else:
            y = monitor["bottom"] - bar_height
        return mon_width, bar_height, x, y

    def _apply_overlay_geometry(self):
        geometry = self._compute_overlay_bar_geometry(self.monitor_index)
        if geometry is None:
            return
