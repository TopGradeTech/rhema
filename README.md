...
- Architecture and conventions are documented in [ARCHITECTURE.md](ARCHITECTURE.md) — worth reading before changing anything, since several decisions look arbitrary until you know what they cost to get wrong. It also covers the branch workflow: work on `development`, and promote via a pull request rather than pushing to `main`.
- `CLAUDE.md` and `AGENTS.md` exist only so coding agents pick up those same conventions automatically. Both are pointers to `ARCHITECTURE.md` and hold no standards of their own, so there is one source of truth to keep current.
